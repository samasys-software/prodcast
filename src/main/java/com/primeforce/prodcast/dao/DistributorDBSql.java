...
public class DistributorDBSql {
    ...
    public final static String GET_ALL_REGISTORS = "select * from registration_details where status='1' or status='0'";
    ...
    public final static String ADMIN_SAVE_DISTRIBUTOR = "insert into dist_dtl (cust_name,customer_type,firstname,lastname,title," +
            ...
            "user_id , open_to_public, fulfillment_type,ip_address, enter_dt_tm , updt_dt_tm, notify_yn," +
            "landmark,manfacture_id , business_id,min_delivery_amt) values ( " +
            "?,?,?,?,?," +
            "?,?,?,?,?," +
            "?,?,?,?,?," +
            "?,?,?,?,?,?," +
    ...
    public final static String ADMIN_UPDATE_DISTRIBUTOR = "update dist_dtl set cust_name=?,customer_type=?,firstname=?," +
            ...
            "email_id=?,cellphone=?,workphone=?,homephone=?,address_1=?," +
            "address_2=?,address_3=?,city=?,state=?,`postal code`=?," +
            "country_id=?,timezone=?,active_yn=?,sex=?,manufacture_yn=?,comments=?,user_id =?,open_to_public=?,fulfillment_type=?,min_delivery_amt=?,updt_dt_tm=NOW() where dist_id=?";

    public final static String ADMIN_UPDATE_SETTINGS = "update my_company_information set salestaxrate=?,companyname=?,address=?,city=?,stateorprovince=?,postalcode=?,country_id=?,phonenumber=?,faxnumber=?,timezone=?,fulfillment_type=?,min_delivery_amt=?,user_id=?,updt_dt_tm=NOW() where dist_id=?";

    public final static String ADMIN_UPDATE_DISTRIBUTOR_SETTINGS = "update dist_dtl set cust_name=?,fulfillment_type=?,min_delivery_amt=?,user_id=?,updt_dt_tm=NOW() where dist_id=?";
    ...
    public final static String CREATE_SETTING_SQL = "insert into my_company_information (dist_id, salestaxrate , companyname , address  , city , stateorprovince,postalcode,country_id,phonenumber,faxnumber,timezone,fulfillment_type,min_delivery_amt,user_id,ip_address,updt_dt_tm)values( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,'',Now())";
  ...
  public final static String ADMIN_EMPLOYEE_ID="select employee_id from employees where email_id=(select email_id from dist_dtl where dist_id=?)";
    ...
    public final static String GET_BRAND_SQL = "select product_brand_id from product_brand where product_brand_name=? and distributor_id=(select dist_manf_id from employees where employee_id = ?)";
